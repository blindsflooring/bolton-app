...
- Live, interactive click-through verification of the Leads screens on real production data — the deploy itself was confirmed stable (all 6 routes live in the real OpenAPI schema, `leads.js` confirmed served by the real frontend, backend booted cleanly against real Postgres including the new cascade-policy entry) and the full lifecycle was verified end-to-end against a throwaway local database, but this session had no real login credentials available to click through it live against Burgert's actual data.
