...
- zone_prices: if the sheet shows multiple zone/region prices for the same product (e.g. Zone A/B/C columns), an object like {"A": 218.00, "B": 228.00, "C": 238.00} — else null. CRITICAL, a real confirmed bug happened from getting this wrong (Como Flooring's price list): some sheets show BOTH a per-m² price AND a per-box price for the SAME zone, right next to each other (e.g. "Zone A: R219.95/m² | R1104.15/box" — two clearly different numbers for the one zone). zone_prices must ALWAYS hold the per-m² figure for each zone, never the per-box one, even if the per-box number is larger, more visually prominent, or listed first/closer to the zone label. If you cannot confidently tell which of two nearby numbers for the same zone is the per-m² one and which is the per-box one, do NOT guess — leave that zone (or the whole product's zone_prices) out and flag "zone_prices" in uncertain_fields instead, exactly like the price/unit_basis rule above: a human reviewer resolving real ambiguity beats this extraction guessing wrong with false confidence.
  SELF-CHECK before finalizing zone_prices, whenever the sheet also shows a per-box price and m2_per_pack alongside the per-m² figures: multiply the per-m² price you're about to record by m2_per_pack — it should land close to the per-box price shown for that same zone (e.g. 219.95 x 5.020 ≈ 1104.15, confirmed real example). If your chosen per-m² number does NOT roughly reproduce the box price this way, you likely have the two numbers backwards — re-read the columns rather than record a value that fails this check.
